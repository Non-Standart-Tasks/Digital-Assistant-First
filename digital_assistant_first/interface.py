...
from digital_assistant_first.geo_system.two_gis import (
    fetch_2gis_data,
    build_route_from_query,
)
from digital_assistant_first.offergen.agent import validation_agent
from digital_assistant_first.offergen.utils import (
    get_system_prompt_for_offers,
    get_system_prompt_for_offers_async,
)
    ...
    for i, message in enumerate(st.session_state["messages"]):
        with st.chat_message(message["role"]):
            ...
            if message["role"] == "assistant":
                ...
                is_map_needed = message.get("request_category") in [
                    "рестораны",
                    "ивенты",
                    "маршруты",
                ] or message.get("show_map", False)
                ...
                if is_map_needed:
                    ...
                    if map_type == "points":
                        # Получаем данные карты из сообщения или из session_state
                        if (
                            i == last_assistant_index
                            and "last_pydeck_data" in st.session_state
                        ):
                    ...
                    # Для маршрутов
                        ...
                        print(
                            f"DEBUG history: Проверка данных маршрута: path_points={len(path_points) if path_points else 0}, route_points={len(route_points) if route_points else 0}"
                        )
                        ...
                        if i == last_assistant_index:
                            if not path_points and "path_points" in st.session_state:
                                ...
                                print(
                                    f"DEBUG history: Взяты path_points из session_state: {len(path_points)} точек"
                                )
                            ...
                            if not route_points and "route_points" in st.session_state:
                                ...
                                print(
                                    f"DEBUG history: Взяты route_points из session_state"
                                )

                        if (
                            path_points
                            and route_points
                            and len(path_points) > 0
                            ...
                            with st.container():
                                ...
                                print(
                                    f"DEBUG history: Данные маршрута: {len(path_points)} точек пути"
                                )
                                ...
                                print(
                                    f"DEBUG history: Координаты центра: {center_lat}, {center_lon}, zoom={zoom_level}"
                                )
                                ...
                                for point in path_points:
                                            ...
                                            segments[current_segment_key].append(
                                                {
                                                    ...
                                                    "name": current_segment_info.get(
                                                        "street_name", ""
                                                    ),
                                                    "speed_type": current_segment_info.get(
                                                        "speed_type", ""
                                                    ),
                                                    "style_type": current_segment_info.get(
                                                        "style_type", ""
                                                    ),
                                                    "style_string": " ("
                                                    + {
                                    ...
                                    elif street_name and not current_segment_info.get(
                                        "street_name"
                                    ):
                                        # Обновляем название улицы, если оно появилось
                                        current_segment_info["street_name"] = (
                                            street_name
                                        )
                                ...
                                for segment_key, paths in segments.items():
                                    ...
                                    if len(parts) >= 2:
                                        ...
                                        style_type = parts[
                                            -1
                                        ]  # Берем последний элемент как стиль
                                    else:
                                    ...
                                    if color_type == "fast":
                                        ...
                                        segment_color = [
                                            ...
                                            0,
                                            200,
                                        ]  # Оранжевый для обычных участков
                                        ...
                                        segment_color = [
                                            ...
                                            0,
                                            200,
                                        ]  # Красный для медленных участков
                                    else:
                                        segment_color = [
                                            0,
                                            0,
                            ...
                            print(
                                f"DEBUG history: Недостаточно данных для отображения маршрута в истории"
                            )
...
def model_response_generator_sync(model, config):
    ...
    try:
        ...
        if request_category == "поездки" or request_category == "другое":
            ...
            tickets_need = loop.run_until_complete(
                aviasales_tool.aviasales_request(model, config, user_input)
            )
            ...
            if tickets_need.get("response", "").lower() == "true":
                ...
                if config.get("aviasales_search") == "True":
                    aviasales_flight_info = loop.run_until_complete(
                        aviasales_tool.get_info_aviasales_url(
        ...
        if config.get("offers_enabled", False):
            try:
                ...
                if (
                    ...
                    != "No relevant offers were found for the search request."
                ):
                    offers_data = {
    ...
    async def check_and_correct_links():
        ...
        if link_statuses.data.links:
            ...
            if some_link_is_invalid:
                ...
                return corrected_answer.data
        return answer

    # Запускаем асинхронный код
    answer = asyncio.run(check_and_correct_links())
...
def handle_user_input_sync(model, config, prompt):
    ...
    if prompt:
        ...
        if pre_category not in ["рестораны", "ивенты", "маршруты"]:
            print(
                f"DEBUG start: Сбрасываем данные карты - запрос не о ресторанах/ивентах/маршрутах"
            )
            st.session_state["show_map"] = False
            st.session_state["last_pydeck_data"] = []
        ...
        with st.chat_message("assistant"):
            ...
            if (
                "aviasales_link" in response
                and response["aviasales_link"]
                and response["aviasales_link"].strip()
            ):
            ...
            if maps_2gis_enabled and response.get("request_category") in [
                "рестораны",
                "ивенты",
            ]:
                ...
                try:
                    ...
                    table_data, pydeck_data = loop.run_until_complete(
                        fetch_2gis_data(prompt, config)
                    )
                    ...
                    if pydeck_data and len(pydeck_data) > 0:
                        ...
                        print(
                            f"Координаты первой точки: lat={pydeck_data[0]['lat']}, lon={pydeck_data[0]['lon']}"
                        )
            ...
            # Если категория запроса - маршруты И включен поиск по 2GIS, получаем данные для построения маршрута
                ...
                try:
                    ...
                    route_info, path_points, points_data, route_details = (
                        loop.run_until_complete(build_route_from_query(prompt, config))
                    )
                    ...
                    print(
                        f"DEBUG: points count: {len(path_points) if path_points else 0}"
                    )
                    ...
                    if route_info and path_points and len(path_points) > 0:
                        ...
                        if (
                            route_details
                            and "instructions_text" in route_details
                            and route_details["instructions_text"]
                        ):
                            route_text += (
            ...
            if config.get("offers_enabled", False):
                ...
                try:
                    ...
                    if offers_system_prompt:
                        ...
                        if (
                            hasattr(offers_response, "content")
                            and offers_response.content
                        ):
                            offers_text = offers_response.content
                        elif (
                            hasattr(offers_response, "message")
                            and offers_response.message.content
                        ):
            ...
            full_response_text = (
                answer_text + places_text + aviasales_text + offers_text
            )
            ...
            if maps_2gis_enabled and st.session_state.get("show_map", False):
                ...
                if (
                    map_type == "points"
                    and st.session_state.get("last_pydeck_data", [])
                    and len(st.session_state["last_pydeck_data"]) > 0
                ):
                    ...
                    map_type == "route"
                    and st.session_state.get("path_points", [])
                    and st.session_state.get("route_points", [])
                ):
            ...
            st.session_state["messages"].append(
                {
                    ...
                    else [],  # Сохраняем данные таблицы
                    "path_points": st.session_state.get(
                        "path_points", []
                    ),  # Сохраняем точки маршрута
                    "route_points": st.session_state.get(
                        "route_points", []
                    ),  # Сохраняем точки начала и конца маршрута
